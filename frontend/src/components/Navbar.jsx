...
function MegaPanel({ categories, defaultCat, onClose }) {
...
export default function Navbar() {
  ...
  const [openMenu, setOpenMenu] = useState(null); // "productos" | "servicios" | null
  ...
  useEffect(() => {
    if (!openMenu) return;
    const handle = (e) => { if (headerRef.current && !headerRef.current.contains(e.target)) setOpenMenu(null); };
  ...
  return (
    <header ref={headerRef} className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled || openMenu ? "bg-white shadow-sm border-b border-gray-100" : "bg-white/90 backdrop-blur-xl"}`} data-testid="navbar">
